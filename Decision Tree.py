...
def main():
    ...
    num_repeats = 30  # 30 repeats to meet strict experimental requirements
    train_frac = 0.7  
    random_seed = 1 

    for current_system in systems:
        # Set the actual path for the dataset
        ...
        if not os.path.exists(datasets_location):
            print(f"Dataset path not found: {datasets_location}, please confirm if the folder location is correct!")
            continue
        ...
        for csv_file in csv_files:
            ...
            # Store test results for baseline model (LR) and Decision Tree model (DT) separately
            metrics_lr = {'MAPE': [], 'MAE': [], 'RMSE': []}
            metrics_dt = {'MAPE': [], 'MAE': [], 'RMSE': []}
            ...
            for current_repeat in range(num_repeats):
                ...
                # 2. New Tool: Decision Tree
                # random_state=42 ensures reproducibility
                dt_model = DecisionTreeRegressor(random_state=42)
                dt_model.fit(training_X, training_Y)
                dt_preds = dt_model.predict(testing_X)

                metrics_dt['MAPE'].append(mean_absolute_percentage_error(testing_Y, dt_preds))
                metrics_dt['MAE'].append(mean_absolute_error(testing_Y, dt_preds))
                metrics_dt['RMSE'].append(np.sqrt(mean_squared_error(testing_Y, dt_preds)))

            # Output and compare results
            lr_avg_mape = np.mean(metrics_lr['MAPE'])
            dt_avg_mape = np.mean(metrics_dt['MAPE'])
            
            print(f"【Average MAPE】 Baseline(LR): {lr_avg_mape:.4f} | New tool(DT): {dt_avg_mape:.4f}")
            print(f"【Average MAE】 Baseline(LR): {np.mean(metrics_lr['MAE']):.2f} | New tool(DT): {np.mean(metrics_dt['MAE']):.2f}")
            print(f"【Average RMSE】 Baseline(LR): {np.mean(metrics_lr['RMSE']):.2f} | New tool(DT): {np.mean(metrics_dt['RMSE']):.2f}")

            # Perform Statistical Test (Wilcoxon Signed-Rank Test)
            stat, p_value = wilcoxon(metrics_lr['MAPE'], metrics_dt['MAPE'])
            print(f"【 p-value】: {p_value:.4e}")
            
            if p_value < 0.05 and dt_avg_mape < lr_avg_mape:
                print("Conclusion: Decision Tree is statistically significantly better than the Baseline model!")
            elif p_value < 0.05 and dt_avg_mape > lr_avg_mape:
                print("Conclusion: Baseline model is better; Decision Tree performed poorly on this dataset.")
            else:
